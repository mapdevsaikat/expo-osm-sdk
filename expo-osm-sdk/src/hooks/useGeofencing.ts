...
export function useGeofencing(
  ...
  const checkGeofences = useCallback(() => {
    ...
    geofences.forEach(geofence => {
      ...
      if (isInside) {
        ...
        if (!wasInside) {
          // ENTER event
          const distance = Math.abs(distanceToGeofence(location, geofence));
  ...
  }, [location, geofences, activeGeofences, geofenceStates, emitEvent]);
  ...
  }, [geofenceStates, dwellThreshold, geofences, location, emitEvent]);
  ...
  }, [location, checkGeofences, checkInterval]);
...
export function useSingleGeofence(
  ...
  const result = useGeofencing([geofence], options);
